...
public class FabricBiome implements IBiome {
    ...
    public float getTemperatureAt(int x, int y, int z)
    {
        ...
        //temperature = this.biomeBase.getTemperature(new BlockPos(x, y, z));
        //but with reflection cus this is private for some reason
        try {
            Class clazz = Biome.class;
            Method m = clazz.getDeclaredMethod("getTemperature", BlockPos.class);
            m.setAccessible(true);
            temperature = (float) m.invoke(this.biomeBase, new BlockPos(x, y, z));
        } catch (IllegalAccessException | NoSuchMethodException | InvocationTargetException e) {
            OTG.getEngine().getLogger().log(LogLevel.ERROR, LogCategory.BIOME_REGISTRY,
                    "Reflection error getting temperature, FabricBiome#getTemperatureAt()");
        }
    ...
    public static Biome createOTGBiome(boolean isOceanBiome, IWorldConfig worldConfig, IBiomeConfig biomeConfig)
    {
        ...
        SoundEvent event = Registry.SOUND_EVENT.get(new ResourceLocation(biomeConfig.getMusic()));
        if (event != null)
        {
        ...
        event = Registry.SOUND_EVENT.get(new ResourceLocation(biomeConfig.getAmbientSound()));
        if (event != null)
        {
            biomeAmbienceBuilder.ambientLoopSound(event);
        }

        event = Registry.SOUND_EVENT.get(new ResourceLocation(biomeConfig.getMoodSound()));
        if (event != null)
        {
        ...
        event = Registry.SOUND_EVENT.get(new ResourceLocation(biomeConfig.getAdditionsSound()));
        if (event != null)
        {
        ...
        BiomeLoadingEvent event1 = new BiomeLoadingEvent(registryName, new Biome.ClimateSettings(rainType, safeTemperature, Biome.TemperatureModifier.NONE, biomeConfig.getBiomeWetness()), category, biomeAmbienceBuilder.build(), genBuilder, spawnBuilder);
        ...
        //Do we need to "set a registry name"? (see forge platform). Biomes are stored in BuiltinRegistries in Fabric
