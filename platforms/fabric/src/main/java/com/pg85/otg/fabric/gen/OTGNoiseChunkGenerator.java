...
public class OTGNoiseChunkGenerator extends ChunkGenerator {
    ...
    private OTGNoiseChunkGenerator(String presetFolderName, String dimConfigName, Registry<NormalNoise.NoiseParameters> noises, BiomeSource biomeProvider1, BiomeSource biomeProvider2, long seed, Supplier<NoiseGeneratorSettings> dimensionSettingsSupplier)
    {
        ...
        this.internalGenerator = new OTGChunkGenerator(this.preset, seed, (ILayerSource) biomeProvider1,((FabricPresetLoader)OTG.getEngine().getPresetLoader()).getGlobalIdMapping(presetFolderName), OTG.getEngine().getLogger());
    ...
    {
        // TODO: implement paper or forge implementation
    }
    ...
    public void buildNoise(StructureFeatureManager manager, ChunkAccess chunk)
    {
        ...
        this.shadowChunkGenerator.queueChunksForWorkerThreads((WorldGenRegion)chunk.getHeightAccessorForGeneration(), manager, chunk, this, (OTGBiomeProvider)this.biomeSource, this.internalGenerator, this.getSettings(), this.preset.getWorldConfig().getWorldHeightCap());

        // If we've already (shadow-)generated and cached this
        // chunk while it was unloaded, use cached data.
        ChunkBuffer buffer = new FabricChunkBuffer((ProtoChunk) chunk);
		...
		for(int xInChunk = 0; xInChunk < Constants.CHUNK_SIZE; ++xInChunk)
		{
			for(int zInChunk = 0; zInChunk < Constants.CHUNK_SIZE; ++zInChunk)
			{
				...
				if(biome.getBiomeConfig().getIsTemplateForBiome())
				{
					...
					((FabricBiome)biome).getBiomeBase().buildSurfaceAt(sharedseedrandom, chunk, worldX, worldZ, i2, d1, ((ForgeMaterialData)biome.getBiomeConfig().getDefaultStoneBlock()).internalBlock(), ((ForgeMaterialData)biome.getBiomeConfig().getDefaultWaterBlock()).internalBlock(), this.getSeaLevel(), 50, worldGenRegion.getSeed());
				}
			}
		}
		*/
    ...
    @Override
    public void applyCarvers(WorldGenRegion p_187691_, long seed, BiomeManager biomeManager, StructureFeatureManager structureFeatureManager, ChunkAccess chunk, GenerationStep.Carving stage)
    {
        ...
        {
            boolean cavesEnabled = true;
            boolean ravinesEnabled = true;
            /* We will always use OTG carvers for now
            FabricBiome biome = (FabricBiome)this.getCachedBiomeProvider().getNoiseBiome(chunk.getPos().x << 2, chunk.getPos().z << 2);
            ...
            boolean cavesEnabled = this.preset.getWorldConfig().getCavesEnabled() && list.stream().anyMatch(
                    a -> defaultCaves.stream().anyMatch(
                            b -> b.equals(
                                    Registry.CARVER.getKey(a.get().worldCarver).toString()
                            )
                    )
            );
            ...
            */
            if(cavesEnabled || ravinesEnabled)
            {
                ...
                ChunkBuffer chunkBuffer = new FabricChunkBuffer(protoChunk);
                /*
	...
	public void applyBiomeDecoration(WorldGenRegion worldGenRegion, StructureFeatureManager structureManager)
	{
		...
		FabricWorldGenRegion forgeWorldGenRegion = new FabricWorldGenRegion(this.preset.getFolderName(), this.preset.getWorldConfig(), (WorldGenRegion)worldGenRegion, this);
		...
		try
		{
			...
			((FabricBiome)biome).getBiomeBase().generate(structureManager, this, worldGenRegion, decorationSeed, sharedseedrandom, blockpos);
			...
			if (getPreset().getWorldConfig().improvedBorderDecoration())
			{
				if (!alreadyDecorated.contains(biome1.getBiomeConfig().getOTGBiomeId()))
				{
					...
					((FabricBiome)biome1).getBiomeBase().generate(structureManager, this, worldGenRegion, decorationSeed, sharedseedrandom, blockpos);
				...
				if (!alreadyDecorated.contains(biome2.getBiomeConfig().getOTGBiomeId()))
				{
					...
					((FabricBiome)biome2).getBiomeBase().generate(structureManager, this, worldGenRegion, decorationSeed, sharedseedrandom, blockpos);
				...
				if (!alreadyDecorated.contains(biome3.getBiomeConfig().getOTGBiomeId()))
				{
					...
					((FabricBiome)biome3).getBiomeBase().generate(structureManager, this, worldGenRegion, decorationSeed, sharedseedrandom, blockpos);
				...
				if (!alreadyDecorated.contains(biome4.getBiomeConfig().getOTGBiomeId()))
				{
					...
					((FabricBiome)biome4).getBiomeBase().generate(structureManager, this, worldGenRegion, decorationSeed, sharedseedrandom, blockpos);
				}
			}
	...
	*/
    ...
    public void spawnOriginalMobs(WorldGenRegion worldGenRegion)
    {
        ...
        NaturalSpawner.spawnMobsForChunkGeneration(worldGenRegion, ((FabricBiome)biome).getBiomeBase(), worldGenRegion.getCenter(), sharedseedrandom);
    }
    ...
    public CustomStructureCache getStructureCache(Path worldSaveFolder)
    {
        ...
        {
            this.structureCache = OTG.getEngine().createCustomStructureCache(this.preset.getFolderName(), worldSaveFolder, this.seed, this.preset.getWorldConfig().getCustomStructureType() == SettingsEnums.CustomStructureType.BO4);
        }
