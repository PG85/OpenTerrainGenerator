...
import com.pg85.otg.fabric.FabricEngine;
...
public class OTGBiomeProvider extends BiomeSource implements ILayerSource {
    ...
    public OTGBiomeProvider(String presetFolderName, Climate.ParameterList<Supplier<Biome>> parameters, Optional<OTGBiomeProvider.PresetInstance> preset)
    {
        ...
        IBiome[] biomeLookup = ((FabricPresetLoader)OTG.getEngine().getPresetLoader()).getGlobalIdMapping(presetFolderName);
        if(biomeLookup == null)
        {
    ...
    private static Stream<Supplier<Biome>> getAllBiomesByPreset(String presetFolderName, WritableRegistry<Biome> registry)
    {
        if(OTG.getEngine().getPluginConfig().getDeveloperModeEnabled())
        {
            ...
            ((FabricEngine)OTG.getEngine()).reloadPreset(presetFolderName, registry);
        } else {
            ...
            ((FabricPresetLoader)OTG.getEngine().getPresetLoader()).reRegisterBiomes(presetFolderName, registry);
        }

        List<ResourceKey<Biome>> biomesForPreset = ((FabricPresetLoader)OTG.getEngine().getPresetLoader()).getBiomeRegistryKeys(presetFolderName);
        if(biomesForPreset == null)
        {
            biomesForPreset= ((FabricPresetLoader)OTG.getEngine().getPresetLoader()).getBiomeRegistryKeys(OTG.getEngine().getPresetLoader().getDefaultPresetFolderName());
        }
        if(biomesForPreset == null)
        {
    ...
    private static Stream<Supplier<Biome>> fixBiomeFeatureOrder(List<ResourceKey<Biome>> biomesForPreset, WritableRegistry<Biome> registry)
    {
        ...
        {
            int itemsRemoved = 0;
            for(Map.Entry<PlacedFeature,List<PlacedFeature>> entry : new HashMap<PlacedFeature,List<PlacedFeature>>(dependenciesPerFeature).entrySet())
            {
