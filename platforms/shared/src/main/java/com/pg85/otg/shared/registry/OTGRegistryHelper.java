...
// All OTG registry bootstrap lives here, NOT in RegistryLoaderMixin: classes in
// the declared mixin package are classload-restricted by Mixin, so any javac
// synthetic (enum-switch $SwitchMap, anonymous class) becomes unloadable once
// the Forgix merge suffix-renames it. Out here renames are harmless.
public final class OTGRegistryHelper {

    private OTGRegistryHelper() {}

    public static void loadOTGPresets(List<Pair<WritableRegistry<?>, Object>> registries, RegistryAccess registryAccess) {
